...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mota.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "motad.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MOTA and zMOTA money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 48882, 48884));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MOTA/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MOTA/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via LightX) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via LightX) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, mota, (obfuscation, lightx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MOTA/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Mota specific functionality (Masternodes, Zerocoin, LightX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:48882"));
    ...
    strUsage += HelpMessageOpt("-backupzmota=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMOTA minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizemotaamount=<n>     " + strprintf(_("Keep N MOTA anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageGroup(_("LightX options:"));
    strUsage += HelpMessageOpt("-enablelightx=<n>", strprintf(_("Enable LightX, show confirmations for locked transactions (bool, default: %s)"), "true"));
    strUsage += HelpMessageOpt("-lightxdepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nLightXDepth));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 48883, 48885));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Mota Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Mota is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enablelightx", fEnableLightX)) {
        if (SoftSetArg("-lightxdepth", 0))
            LogPrintf("AppInit2 : parameter interaction: -enablelightx=false -> setting -nLightXDepth=0\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Mota is shutting down."));
...
#endif
    // Make sure only a single Mota process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mota is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Mota: load previous sessions sporks if we have them.
                ...
                // Mota: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Mota") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Mota to complete") << "\n";
        ...
        bool fEnableZMotaBackups = GetBoolArg("-backupzmota", true);
        pwalletMain->setZMotaAutoBackups(fEnableZMotaBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeMotaAmount = GetArg("-anonymizemotaamount", 0);
//    if (nAnonymizeMotaAmount > 999999) nAnonymizeMotaAmount = 999999;
//    if (nAnonymizeMotaAmount < 2) nAnonymizeMotaAmount = 2;

    fEnableLightX = GetBoolArg("-enablelightx", fEnableLightX);
    nLightXDepth = GetArg("-lightxdepth", nLightXDepth);
    nLightXDepth = std::min(std::max(nLightXDepth, 0), 60);
    ...
    LogPrintf("nLightXDepth %d\n", nLightXDepth);
    LogPrintf("Anonymize Mota Amount %d\n", nAnonymizeMotaAmount);
    ...
    /* Denominations
       ...
       For example:
       1MOTA+1000 == (.1MOTA+100)*10
       10MOTA+10000 == (1MOTA+1000)*10
    */
