                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMOTA ought to be enough for anybody." - Bill Gates, 2017
    ui->zMOTApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMOTASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMOTA->setEnabled(false);
        ui->pushButtonMintzMOTA->setToolTip(tr("zMOTA is currently disabled due to maintenance."));

        ui->pushButtonSpendzMOTA->setEnabled(false);
        ui->pushButtonSpendzMOTA->setToolTip(tr("zMOTA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMOTApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMOTA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMOTA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMOTA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMOTA in ") + 
...
void PrivacyDialog::on_pushButtonSpendzMOTA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMOTA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMOTA
        sendzMOTA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMOTA
    sendzMOTA();
}

void PrivacyDialog::on_pushButtonZMotaControl_clicked()
{
    ZMotaControlDialog* zMOTAControl = new ZMotaControlDialog(this);
    zMOTAControl->setModel(walletModel);
    zMOTAControl->exec();
}

void PrivacyDialog::setZMotaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMOTASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMOTA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Mota Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMOTApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMOTA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MOTA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMOTA</b>";
    ...
    // use mints from zMOTA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMotaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMotaControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmota selector in case it was used
    ZMotaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMOTA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Mota, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMOTA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMOTA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMOTA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMOTA "));
    ui->labelzMOTAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMOTASyncStatus->setVisible(fShow);
}
