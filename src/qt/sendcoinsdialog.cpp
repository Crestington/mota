                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    if (!settings.contains("bUseLightX"))
        settings.setValue("bUseLightX", false);

    bool useLightX = settings.value("bUseLightX").toBool();
    if (fLiteMode) {
        ui->checkLightX->setVisible(false);
        CoinControlDialog::coinControl->useObfuScation = false;
        CoinControlDialog::coinControl->useLightX = false;
    } else {
        ui->checkLightX->setChecked(useLightX);
        CoinControlDialog::coinControl->useLightX = useLightX;
    }

    connect(ui->checkLightX, SIGNAL(stateChanged(int)), this, SLOT(updateLightX()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkLightX->isChecked()) {
        recipients[0].useLightX = true;
        strFunds += " ";
        strFunds += tr("and LightX");
    } else {
        recipients[0].useLightX = false;
    }
...
void SendCoinsDialog::updateLightX()
{
    QSettings settings;
    settings.setValue("bUseLightX", ui->checkLightX->isChecked());
    CoinControlDialog::coinControl->useLightX = ui->checkLightX->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee2->setText(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::minTxFee.GetFeePerK()) + "/kB" + (tr(" Smart fee not initialized yet. This usually takes a few blocks...")));
    } else {
        ui->labelSmartFee2->setText(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB" + (tr(" Estimated to begin confirmation within %n block(s).", "", nBlocksToConfirm)));
    }
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Mota address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
