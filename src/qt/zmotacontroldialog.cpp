...
#include "ui_zmotacontroldialog.h"
...
std::list<std::string> ZMotaControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMotaControlDialog::listMints;

ZMotaControlDialog::ZMotaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMotaControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMotaControlDialog::~ZMotaControlDialog()
{
    delete ui;
}

void ZMotaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMotaControlDialog::updateList()
{
...
void ZMotaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMotaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMota_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMotaControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMotaControlDialog::GetSelectedMints()
{
...
void ZMotaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
