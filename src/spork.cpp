...
// Mota: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_2_LIGHTX) r = SPORK_2_LIGHTX_DEFAULT;
        if (nSporkID == SPORK_3_LIGHTX_BLOCK_FILTERING) r = SPORK_3_LIGHTX_BLOCK_FILTERING_DEFAULT;
...
{
    if (strName == "SPORK_2_LIGHTX") return SPORK_2_LIGHTX;
    if (strName == "SPORK_3_LIGHTX_BLOCK_FILTERING") return SPORK_3_LIGHTX_BLOCK_FILTERING;
...
{
    if (id == SPORK_2_LIGHTX) return "SPORK_2_LIGHTX";
    if (id == SPORK_3_LIGHTX_BLOCK_FILTERING) return "SPORK_3_LIGHTX_BLOCK_FILTERING";
